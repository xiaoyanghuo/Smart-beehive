...
moter = MOTer(modelpath='finalbest.onnx')
...
def InOutjudge(db, cursor, tracker, time):
    ...
    if timer % max_time == 0:
        if send:
            insert_on_quantityrecord(db, cursor,
                                     [1, time, bugs_in[0] + bugs_in[3], bugs_out[0] + bugs_in[3]])
...
def get_cursor():
    db = pyodbc.connect(
        'DRIVER={MySQL ODBC 8.0 ANSI Driver};SERVER=192.168.43.206;DATABASE=beehive;UID=beehive1;PWD=123'
    )
    # db = pyodbc.connect(
        ...
        if len(r_list) > 0:  # 套接字可以读取
            ...
            if len(data) < 200:
                ...
                if len(jsondata.keys()) == 2:
                    ...
                    print(location)
                    if send:
                        update_on_beehives(db, cursor,
                                           [1, "POINT(" + str(location["longitude"]) + "," + str(location[
                                                                                                     "latitude"]) + ")"])
                #  温湿度模块
                    ...
                    print(compo)
                    if send:
                        insert_on_humidityandtemperaturerecord(db, cursor,
                                                               [1, other_way_time, compo['humidity'],
                                                                compo['temperature']])
            ...
            # 图像处理
                ...
                if timer != 1:
                    ...
                    cv.imshow('window', np.zeros((640,640,3)))
            if send:
                close_db(db, cursor)
